...
export const TreatmentPage = ({ data }: Props) => {
  const { language } = useLanguage();

  return (
    <>
      <SEOHead title={data.seo.metaTitle} description={data.seo.metaDescription} path={`/dog-dental-disease/treatments/${data.slug}`} language={language} />
      <DentalSchemaMarkup page={data} breadcrumbItems={[
        ...
        { name: 'Treatments', url: '#' },
        { name: data.title, url: `https://petcaremalaysia.com/dog-dental-disease/treatments/${data.slug}` },
      ]} />
      <Header />
      <main className="min-h-screen bg-background">
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <section className="mb-8">
            <h2 className="text-2xl font-bold mb-3">When Veterinarians Recommend It</h2>
          ...
          <section className="mb-8">
            ...
            <ul className="list-disc pl-5 space-y-1">
              {data.benefits.map((b, i) => <li key={i} className="text-muted-foreground">{b}</li>)}
            </ul>
          </section>

          <section className="mb-8">
            <h2 className="text-2xl font-bold mb-3">Risks & Considerations</h2>
            <p className="text-muted-foreground">{data.risksAndConsiderations}</p>
          </section>

          <section className="mb-8">
            <h2 className="text-2xl font-bold mb-3">Follow-Up Care</h2>
            <p className="text-muted-foreground">{data.followUpCare}</p>
          </section>
