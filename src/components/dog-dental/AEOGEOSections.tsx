...
import type { DentalPageBase, DentalCitation } from '@/data/dog-dental/types';

interface AEOGEOProps {
  data: DentalPageBase;
  /** Where to place snippet: 'top' renders snippet+tldr+quickFacts, 'bottom' renders stats+cost+malaysia+citations+takeaways */
  position: 'top' | 'bottom';
}

export const AEOGEOSections = ({ data, position }: AEOGEOProps) => {
  if (position === 'top') {
    return (
      <>
        {/* Snippet for AI — hidden visually but crawlable */}
        {data.snippetForAI && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-muted/50">
                  ...
                  <th className="text-left p-3 font-semibold text-foreground">Price Range (RM)</th>
                </tr>
              </thead>
              <tbody>
      ...
      {data.citations && data.citations.length > 0 && (
        <section className="mb-8">
          ...
          <ol className="space-y-3 list-decimal pl-5">
            {data.citations.map((c: DentalCitation, i: number) => (
